  ...
  const getStatusBadge = (status: AppraisalCycleStatus) => {
    switch (status) {
      case AppraisalCycleStatus.ACTIVE:
        return (
          <span className="px-2 py-1 text-xs font-medium bg-green-100 text-green-800 dark:bg-green-950 dark:text-green-200 rounded-full flex items-center gap-1">
      ...
      case AppraisalCycleStatus.PLANNED:
        return (
          <span className="px-2 py-1 text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-950 dark:text-blue-200 rounded-full flex items-center gap-1">
          ...
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6 shadow-sm">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="text-center">
                <p className="text-2xl font-bold text-gray-900 dark:text-white">{cycles.length}</p>
                <p className="text-sm text-gray-500 dark:text-gray-300">Total Cycles</p>
              </div>
              <div className="text-center">
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-300">Planned</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-bold text-gray-600 dark:text-gray-300">
                  {cycles.filter((c) => c.status === AppraisalCycleStatus.ARCHIVED).length}
                </p>
                <p className="text-sm text-gray-500 dark:text-gray-300">Archived</p>
              </div>
            </div>
          </div>
        )}
              ...
              >
                    ...
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        ...
                        <div className="flex items-center space-x-2 mt-1">
                          ...
                          <span className="px-2 py-1 text-xs font-medium bg-purple-100 text-purple-800 dark:bg-purple-950 dark:text-purple-200 rounded-full">
                            {getCycleTypeLabel(cycle.cycleType)}
                          </span>
                        </div>
                        {cycle.description && (
                          <p className="text-sm text-gray-500 dark:text-gray-300 mt-1">{cycle.description}</p>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-3">
                    ...
                    <div className="flex space-x-2">
                      <Link href={`/performance/cycles/view/${cycle._id}`}>
                        <button className="p-2 text-gray-600 dark:text-gray-300 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-gray-700 rounded-md" title="View">
                          <Eye size={16} />
                        </button>
                      </Link>
                      <Link href={`/performance/cycles/edit/${cycle._id}`}>
                        <button className="p-2 text-gray-600 dark:text-gray-300 hover:text-green-600 hover:bg-green-50 dark:hover:bg-gray-700 rounded-md" title="Edit">
                          <Edit size={16} />
                        </button>
                      </Link>
                      <button
                        className="p-2 text-gray-600 dark:text-gray-300 hover:text-purple-600 hover:bg-purple-50 dark:hover:bg-gray-700 rounded-md"
                        title="Duplicate"
                ...
                <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                  ...
                  <Link href={`/performance/analytics?cycleId=${cycle._id}`}>
                    <button className="px-3 py-1 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-200 text-sm rounded-md hover:bg-gray-50 dark:hover:bg-gray-700 flex items-center gap-1">
                      <BarChart size={12} />
                      Analytics
                    </button>
                  </Link>
